...
const Header = () => {
  ...
  return (
    <div className="w-full h-20 border-b-[1px] border-gray-500 px-4 bg-white shadow-md">
      <div className="h-full max-w-screen-2xl mx-auto flex items-center justify-between">
        {/* Logo */}
        <Link href={"/"} className="relative group overflow-hidden">
          <p className="text-2xl font-bold text-gray-800 transition-all duration-300 hover:text-primeColor">Digital Spark</p>
        ...
        <div className="hidden lg:inline-flex items-center gap-8 text-sm uppercase font-semibold tracking-wide">
          <ul className="flex gap-8">
            {headerListItem.map((item) => (
              <Link key={item._id} href={item.link}>
                <li
                  className={`${
                    active === item.link ? "text-primeColor" : "text-gray-800"
          ...
          <button className="w-36 h-10 bg-darkGreen text-white uppercase rounded-md hover:bg-primeColor duration-300 transition-colors">
            Hire me
          </button>
        </div>
      </div>
